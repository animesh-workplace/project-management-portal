...
class ModuleModelSchema(models.Model):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._initial_model_name = self.model_name
...
class ModuleFieldSchema(models.Model):
    _PROHIBITED_NAMES = ("__module__", "_schema", "_declared")
    _MAX_LENGTH_DATA_TYPES = ("character",)

    name = models.CharField(max_length=63)
    module_schema = models.ForeignKey(
        ModuleModelSchema, on_delete=models.CASCADE, related_name="fields"
    )
    ...
    class Meta:
        unique_together = (("name", "module_schema"),)
    ...
    def get_registered_model_field(self):
        latest_model = self.module_schema.get_registered_model()
    ...
    def update_last_modified(self):
        # self.sample_identifier.last_modified = timezone.now()
        self.module_schema.last_modified = timezone.now()
    ...
    def _get_model_with_field(self):
        model = self.module_schema.as_model()
        try:
...
class Modulename(models.Model):
    projectname = models.ForeignKey(
        Projectname, verbose_name="modelname", on_delete=models.CASCADE
    )
