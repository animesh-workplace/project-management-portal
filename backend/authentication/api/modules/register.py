...
class RegisterSerializer(serializers.Serializer):
    ...
    def create(self, validated_data):
        ...
        activation_link = (
            f"{os.getenv('BASE_LINK')}{os.getenv('BASE_URL')}auth/activation?username={instance.username}"
            f"&email={instance.email}&token={token}"
        )
        send_email_activate(
            instance.first_name, instance.username, instance.email, activation_link
        )
        return instance
...
class RegisterView(generics.CreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            serializer.save()
            return Response(
                {"message": "Registered Successfully, check mail", "code": "SUCCESS"}
            )
        return Response(
            create_uniform_response(serializer.errors),
