...
class ForgotPasswordSerializer(serializers.Serializer):
    ...
    def validate_username_email(self, username, email):
        if(email):
            ...
            raise serializers.ValidationError({'Error': 'Must include either username or email'}, code='natural')
        return user
    ...
    def validate(self, value):
        ...
        try:
            ...
            link = (f"{os.getenv('BASE_LINK')}{os.getenv('BASE_URL')}auth/verify?username={instance.username}"
                    f"&email={instance.email}&token={token}")
            send_email_reset_password(instance.first_name, instance.username, instance.email, link)
            return value
        except IntegrityError:
            raise serializers.ValidationError({'Error': 'Mail already sent'}, code='natural')
