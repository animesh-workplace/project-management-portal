...
class RequestMailChangeSerializer(serializers.Serializer):
    ...
    def validate(self, value):
        ...
        if(old_email == new_email):
            raise serializers.ValidationError({'Error': 'Both mail are identical'}, code='natural')
        instance = self.context['request'].user
        if(instance.email == old_email):
            try:
                token = MailToken.objects.create(user=instance, token_type=2)
                link = (f"{os.getenv('BASE_LINK')}{os.getenv('BASE_URL')}activate?username={instance.username}"
                        f"&old_email={instance.email}&new_email={new_email}&token={token}")
                send_email_mail_change_request(instance.first_name, instance.username, old_email, new_email, link)
                return value
