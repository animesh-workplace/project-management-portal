<template>
	<div>
		...
		<div class="grid grid-flow-col col-span-13">
			...
			<div v-if="sample_identifier" class="col-span-12 pt-4 pr-2 flex justify-right box-content h-full w-90 p-4 border-4 grid grid-flow-col col-span-2">
				<div>
					<form>
					    ...
					    <div class="grid gap-6 mb-6 lg:grid-cols-1">
					        <div v-for="event in projectConfig" class="capitalize">
								...
								<div v-if="event.data_type=='filefield'">
								    <input v-on:change="handleFileupload($event)" type="file" class="block w-full text-sm text-gray-500 file:py-2 file:px-6 file:rounded file:border-1 file:border-gray-400"/>
								</div>
				...
				<div class="ml-4">
					<CreateBulkUploadToProjects />
				</div>
			</div>
			<div v-if="metadata" class="col-span-12 pt-4 pr-2 flex justify-right box-content h-full w-90 p-4 border-4 grid grid-flow-col col-span-2">
				<div>
					<form>
					    <div class="grid gap-6 mb-6 lg:grid-cols-1">
					        <div class="capitalize" v-for="event in metadataConfig">
					            <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">{{event.name.replaceAll("_", " ")}}</label>
					            <input v-model="params[event.name.toLowerCase()]" type="text" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-80 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="" required v-if="event.data_type=='character'">
					            <input v-model="params[event.name.toLowerCase()]" type="number" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-80 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="" required v-if="event.data_type=='integer'">
								<div v-if="event.data_type=='radio'">
									<ul class="items-center w-full text-sm font-medium text-gray-900 rounded-lg border border-gray-200 sm:flex dark:bg-gray-700 dark:border-gray-600 dark:text-white">
									    <li v-for="i in event.options" class="w-full border-b border-gray-200 sm:border-b-0 sm:border-r dark:border-gray-600">
									        <div class="flex items-center pl-3">
									            <input v-model="params[event.name.toLowerCase()]" :id="event.name.toLowerCase()" type="radio" :value="i" :name="event.name.toLowerCase()" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500">
									            <label :for="event.name.toLowerCase()" class="py-3 ml-2 w-full text-sm font-medium text-gray-900 dark:text-gray-300">{{i}}</label>
									        </div>
									    </li>
									</ul>
								</div>
	...
	export default {
		...
		methods: {
			handleFileupload(event) {
				// const blob = new Blob([event.target.files[0]], {type: "multipart/form-data"});
				// this.file = event.target.files[0]
				const sfiles = {}
				this.projectConfig.forEach(function(i){
					if(i["data_type"]=="filefield"){
						sfiles[i["name"]] = event.target.files[0]
					}
				})
				this.files = sfiles
			},
			...
			async uploadProject() {
				...
				Object.keys(this.files).forEach(key => formData.append(key, this.files[key]));
				Object.keys(this.params).forEach(key => formData.append(key, this.params[key]));
		        try {
		            const res = await this.$axios.post("http://10.10.6.87/pmp/api/schema/projects/upload/", formData, {headers: {
			...
			async uploadProject1() {
				console.log("params",this.params)
				this.$store.dispatch('base/UploadProject', {"name": this.$route.query.name, "data": [this.params]})
			},
			async uploadMetadata() {
				this.$store.dispatch('base/UploadMetadata', {name: `${this.username}_${this.$route.query.name.split("_")[1]}_${this.metadata_name}_metadata`, data: [this.params]})
			}
		},
		mounted() {
	    	this.$nextTick(() => {
