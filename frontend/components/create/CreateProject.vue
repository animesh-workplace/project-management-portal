<template>
	<div>
		<div class=" grid grid-cols-1 gap-4 place-items-center">
			<form>
			    <h1 class='text-center text-xl font-bold'>Create you project</h1>
			    <div>
	        	...
	        	<h2 class='text-center text-blue-400 text-xl font-bold mt-2' id="field1">Create clinical data structure format</h2>
		        <div v-for="(element, index) in params.config">
		        	<h2 class='text-center text-xl font-bold mt-2' id="field1">Sample index register {{index+1}}</h2>
				    <div>
					...
					<div v-if="((element['data_type']=='multiradio') || (element['data_type']=='radio'))">
						...
						<button v-if="((element['data_type']=='multiradio') || (element['data_type']=='radio'))" class="text-white bg-gray-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" @click="onOptions(element)" type="button">Add</button>
			        </div>
			...
			async createSampleIdentifier() {
				this.params.config.forEach(function(e,i) {
					...
					if ((e.data_type == "radio") || (e.data_type == "multiradio")) {
					    ...
					    e.options = optionsList
					}
				})
				await this.$store.dispatch("base/CreateSampleIdentifier", this.params);
			},
		}
	}
</script>