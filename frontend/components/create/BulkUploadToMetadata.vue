	...
	<div>
		<form>
			...
			<div class="grid grid-flow-col col-span-2 gap-2">
		  	...
		  	<button @click="uploadMetadata()" type="button" class="mt-2 text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Submit SI</button>
	...
	export default {
		...
		methods: {
			...
			async uploadMetadata() {
				...
				await this.$store.dispatch('base/UploadMetadata', {name: `${this.$route.query.name.split("_")[0]}_${this.$route.query.name.split("_")[1]}_${this.$route.query.metadata}_metadata`.toLowerCase(), data: this.newData})
			},
			async downloadMetadataTemplate() {
		      const csv = await this.$axios.post(`http://10.10.6.87/pmp/api/schema/metadata/template/`, {project: this.$route.query.name.split("_")[1], name: this.$route.query.metadata.toLowerCase()})
		      console.log(csv)
		      const file_name = csv.data.split("/").at(-1);
		      const download_path = `http://10.10.6.87/pmp/api/metadata/download/${file_name}`;
		      this.$axios({
		        url: download_path,
		        method: 'GET',
		        responseType: 'blob',
